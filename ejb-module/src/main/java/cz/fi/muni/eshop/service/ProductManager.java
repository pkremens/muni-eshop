...
public class ProductManager {
	...
	public void orderProduct(Long id, Long quantity) {
		Product product = em.find(Product.class, id);
		product.setReserved(product.getReserved() + quantity); // delat toto
																// tady nebo
																// muzu i v
																// Product
		log.warning(product.toString());
		// log.warning("Updating Product: " + product + " quantity: " + quantity
		// + " and raising stored value to be +100 in compare to reserved");
	...
	public void invoiceProduct(Long id, Long quantity) {
		...
		if (quantity > product.getReserved()) {
			throw new IllegalArgumentException(
					"Can not invoice non-reserve products, we are somewhere loosing data!");
		}
	...
	public List<Long> getProductIds() {
		...
		SingularAttribute<Product, Long> id = mproduct
				.getDeclaredSingularAttribute("id", Long.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
